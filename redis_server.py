...
def wait_for_sync(master_conn, slave_conn):
    identifier = str(uuid.uuid4())
    master_conn.zadd('sync:wait', {identifier: time.time()})  # A

    while not slave_conn.info()['master_link_status'] != 'up':  # B
        time.sleep(.001)

    while not slave_conn.zscore('sync:wait', identifier):  # C
        time.sleep(.001)

    deadline = time.time() + 1.01  # D
    while time.time() < deadline:  # D
        if slave_conn.info()['aof_pending_bio_fsync'] == 0:  # E
            break  # E
        time.sleep(.001)

    master_conn.zrem('sync:wait', identifier)  # F
    master_conn.zremrangebyscore('sync:wait', 0, time.time() - 900)  # F
